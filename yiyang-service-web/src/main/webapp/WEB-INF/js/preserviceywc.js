...
$(function () {
    ...
    $selectywclist.click(function () {
        ...
        $.post("/item/ywclist",{page:nowpage,rows:14,psUname:psUname3,psTelnumber:psTelnumber3},function(data){
            ...
            console.log(pagenum);
            var string=" <table class=\"table table-condensed table-striped\" id=\"mytable3\" title=\"已完成工单\">" +
                "<thead id=\"thead\">" +
            ...
            for(var i=0;i<list.length;i++)
            {
                string+="<tr>"+"<td ><input type=\"checkbox\"></td>\n"+
                    ...
                    "<td>"+list[i].psFlag+"</td>" +
                    "<td><a href=\"#\" onclick='show(this)'>查看详情</a> </td>" +
                    "</tr>"
            }
            string+="</table>";
          $("#form3").after(string);
...
function show(obj) {
    ...
    var psNumber = $("#mytable3 tr:eq(" + rows + ") td:eq(2)").html();
    ...
    function pageclick(obj) {
        ...
        $.post("/item/ywclist", {page: nowpage, rows: 14,psUname:psUname3,psTelnumber:psTelnumber3}, function (data) {
            var string = " <table class=\"table table-condensed table-striped\" id=\"mytable3\" title=\"已完成工单\">" +
                "<thead id=\"thead\">" +
                "<tr><th ><input checked=\"checked\" type=\"checkbox\" id=\"selectAll\"></th><th>序号</th> <th>订单编号</th><th >客户姓名</th><th >电话号码</th><th>下单时间</th><th>要求时间</th><th >服务地址</th><th >完成时间</th><th >服务状态</th><th >操作</th></tr>" +
                "</thead>";
            var list = data.rows;
            for (var i = 0; i < list.length; i++) {
                string+="<tr>"+"<td ><input type=\"checkbox\"></td>\n"+
