...
$(function () {
    ...
    $("#hlist").click(function () {
        ...
        $("#mytable").remove();
        $("#pt").remove();
        var infoName=$("#sitem").val();
        $.post("/relation/preremind",{page:nowpage,rows:14,infoName:infoName},function(data){
...
function addelement(data) {
    ...
    var string=" <table class=\"table table-condensed table-striped table-bordered\" id=\"mytable\" title=\"待处理工单\">" +
        "<thead><td>序号</td><td>姓名</td><td>预警项目</td><td>预警值</td><td>预警时间</td><td>处理状态</td><td>操作</td></thead>"
    var list=data.rows;
    console.log(list)
...
function changeflag(obj) {
    ...
    $.post("/relation/updateflag",{prId:recId},function (data) {
